...
class SVEA_BC(SAC):
	def __init__(self, obs_shape, action_shape, args):
		...
		self.pad_head = m.BehaviorCloning(self.aux_encoder, action_shape, args.hidden_dim).cuda()
	...
	def reset(self, step):
		...
		if self.args.do_policy_reset:
			...
			print('Performed BC head reset at step ', step)
	...
	def update_behavior_cloning(self, obs, action, L=None, step=None):
		assert obs.shape[-1] == 84

		pred_action = self.pad_head(obs)
	...
	def update(self, replay_buffer, L, step):
		...
		if step % self.aux_update_freq == 0:
			self.update_behavior_cloning(obs, action, L, step)
